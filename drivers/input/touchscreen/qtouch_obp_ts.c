...
/*add touch debug information*/
#ifdef CONFIG_TOUCHSCREEN_DEBUG
static uint32_t counter_irq;
static uint32_t counter_workqueue;
static struct coordinate_map copy_finger_data[_NUM_FINGERS];
static char last_wq_time[64];
...
/* variables for touch filter */
static int last_reported_x[_NUM_FINGERS] = {0,};
static int last_reported_y[_NUM_FINGERS] = {0,};
static int last_position_x[_NUM_FINGERS] = {0,};
static int last_position_y[_NUM_FINGERS] = {0,};
...
static int qtouch_hw_init(struct qtouch_ts_data *ts)
{
	...
	/* configure the palm suppression table (T41) */
	...
	/* configure the Digitizer HID config (T43) */
	...
	/* configure noise suppression 3 (T54) */
	obj = find_obj(ts, QTM_OBJ_PROCG_NOISE_SUPPRESSION3);
	if (obj && obj->entry.num_inst > 0) {
		ret = qtouch_write_addr(ts, obj->entry.addr,
				    &ts->pdata->noise_suppression3_cfg,
				    min(sizeof
				        (ts->pdata->noise_suppression3_cfg),
				        obj->entry.size));
		if (ret != 0) {
			pr_err("%s: Can't write noise3 config\n", __func__);
			return ret;
		}
	}
			   ...
			   struct qtm_object *obj, void *_msg)
{
	...
	real_y = y;
	if (qtouch_tsdebug & 2)
		pr_info("%s: IC stat=%02x, f=%d x=%d y=%d p=%d w=%d\n",
			__func__, msg->status, finger, x, y, pressure, width);

	/* if this is the first touch down reported */
	if ((last_reported_x[finger] == 0) && (last_reported_y[finger] == 0) &&
	    (last_position_x[finger] == 0) && (last_position_y[finger] == 0)) {
		last_reported_x[finger] = x;
		last_position_x[finger] = x;
		last_reported_y[finger] = y;
		last_position_y[finger] = y;
		/* indicate that this is the first touch and it should be
		 * reported, even after Hystersis */
		report = 2;
	}

	/* add some new levels to the filter to reduce latency on fast
	   movements*/
	if (abs(x-last_reported_x[finger]) > 95)
		last_position_x[finger] = x;
	else if (abs(x-last_reported_x[finger]) > 47)
		last_position_x[finger] = (x + last_position_x[finger]) >> 1;

	if (abs(y-last_reported_y[finger]) > 95)
		last_position_y[finger] = y;
	else if (abs(y-last_reported_y[finger]) > 47)
		last_position_y[finger] = (y + last_position_y[finger]) >> 1;

	/* if the current x position and the last x position are both greater
	 *  than the last reported position */
	if ((x > last_reported_x[finger]) &&
	    (last_position_x[finger] > last_reported_x[finger])) {
		/* then if current position is bigger than the last position */
		if (x > last_position_x[finger]) {
			/* then make the new reported position halfway between
			 * the last reported position and the last position */
			temp = (last_position_x[finger]
				+ last_reported_x[finger]) >> 1;
			last_position_x[finger] = x;
		} else {
			/* if last_position_x[finger] >= x then make the
			 * new reported position halfway between the
			 * current position and the last reported position */
			temp = (x + last_reported_x[finger]) >> 1;
			last_position_x[finger] = x;
		}
		x = temp;
	} else if ((x < last_reported_x[finger]) &&
		   (last_position_x[finger] < last_reported_x[finger])) {
		/* if the current x position and the last x position are both
		 * less than the last reported position.
		 * then if current position is further from the last reported
		 * position */
		if (x < last_position_x[finger]) {
			/* then make the new reported position halfway between
			 * the last reported position and the last position */
			...
			/* if last_position_x[finger] is further from the last
			 * reported position then make the new reported position
			 * halfway between the current position and the last
			 * reported position */
			temp = (x + last_reported_x[finger]) >> 1;
			last_position_x[finger] = x;
		}
		...
		/* else, store last position and do not change the position */
		last_position_x[finger] = x;
		x = last_reported_x[finger];
	}
	/* if the current y position and the last y position are both greater
	 * than the last reported position */
	if ((y > last_reported_y[finger]) &&
	    (last_position_y[finger] > last_reported_y[finger])) {
		/* then if current position is bigger than the last position */
		if (y > last_position_y[finger]) {
			/* then make the new reported position halfway between
			 * the last reported position and the last position */
			temp = (last_position_y[finger]
				+ last_reported_y[finger]) >> 1;
			last_position_y[finger] = y;
		} else {
			/* if last_position_y[finger] >= y then make the new
			 * reported position halfway between current position
			 * and the last reported position */
			temp = (y + last_reported_y[finger]) >> 1;
			last_position_y[finger] = y;
		}
		y = temp;
	} else if ((y < last_reported_y[finger]) &&
		   (last_position_y[finger] < last_reported_y[finger])) {
		/* if the current y position and the last y position are both
		 * less than the last reported position.
		 * then if the current position is further from the last
		 * reported position */
		if (y < last_position_y[finger]) {
			/* then make the new reported position halfway between
			 * the last reported position and the last position */
			...
			/* if last_position_y[finger] is further from the last
			 * reported position then make the new reported position
			 * halfway between the current position and the last
			 * reported position */
			temp = (y + last_reported_y[finger]) >> 1;
		...
		/* else, do not change the position */
		last_position_y[finger] = y;
		y = last_reported_y[finger];
	}

	/* Hysteresis Filter
	 * if x has moved less than 3 positions, do not change the position
	 * if y has moved less than 3 positions, do not change the position */
	if (((x - last_reported_x[finger]) <= 3) &&
	    ((last_reported_x[finger] - x) <= 3))
		x = last_reported_x[finger];
	if (((y - last_reported_y[finger]) <= 3) &&
	    ((last_reported_y[finger] - y) <= 3))
		y = last_reported_y[finger];

	/* Do not report the message if Hysteresis has been applied in both
	 * directions and it not the first touch*/
	if (x == last_reported_x[finger] &&
	    y == last_reported_y[finger] && report == 1)
		report = 0;
	else
		report = 1;

	/* store last reported position */
	last_reported_x[finger] = x;
	last_reported_y[finger] = y;
	...
	/* Driver filter need to report release position as pressed touch */
	if (!down) {
		diff_position_x = abs(real_x - x);
		diff_position_y = abs(real_y - y);

		if (diff_position_x > 20 || diff_position_y > 20) {
			ts->finger_data[finger].z_data = pressure;
			ts->finger_data[finger].w_data = width;

			if (diff_position_x > 50 || diff_position_y > 50) {
			...
			for (i = 0;
				...
				if (ts->finger_data[i].down == 0)
					continue;
#ifdef CONFIG_TOUCHSCREEN_DEBUG
				memset(&copy_finger_data[i], 0,
				sizeof(struct coordinate_map));
				memcpy(&copy_finger_data[i],
				       &ts->finger_data[i],
				       sizeof(struct coordinate_map));
#endif
				if (qtouch_tsdebug & 2)
					pr_info("%s: report x %i y %i\n",
						__func__,
	...
	if (!down) {
		...
		ts->finger_data[finger].x_data = real_x;
		ts->finger_data[finger].y_data = real_y;
		ts->finger_data[finger].down = 0;
		/* if hysteresis was applied but this is a release, report it */
		report = 1;
		/* reset filter values */
		last_reported_x[finger] = 0;
		last_position_x[finger] = 0;
		last_reported_y[finger] = 0;
		last_position_y[finger] = 0;

		if (ts->pdata->flags & QTOUCH_USE_KEYARRAY) {
			if (ts->finger_data[finger].y_data >
			   ((9 * ts->pdata->abs_max_y) / 10)) {
				ignore_keyarray_touches = 1;
				mod_timer(&keyarray_timer, jiffies +
					  KEYARRAY_IGNORE_TIME);
				if (qtouch_tsdebug)
	...
	if (report) {
		for (i = 0; i < ts->pdata->multi_touch_cfg.num_touch; i++) {
			...
			memset(&copy_finger_data[i], 0,
			sizeof(struct coordinate_map));
			memcpy(&copy_finger_data[i], &ts->finger_data[i],
			sizeof(struct coordinate_map));
#endif
			     ...
			     struct qtm_obj_message *msg)
{
	...
	case QTM_OBJ_TOUCH_MULTI:
		if (ts->family_id == QTM_OBP_TABLET_FAMILY_ID)
			ret = qtouch_do_touch_multi_msg_filter(ts, obj, msg);
		else
...
static int qtouch_ts_register_input(struct qtouch_ts_data *ts)
{
	...
	if (obj && obj->entry.num_inst > 0) {
		...
		input_set_abs_params(ts->input_dev, ABS_MT_TRACKING_ID,
				     0, ts->pdata->multi_touch_cfg.num_touch - 1,
				     0, 0);
	}
...
touch_to_normal_mode:
	/*
	 * This is a COMPLETE HACK! Disable the verbose logging enabled in the
	 * probe function to work around voltage drop issues.
	 */
	qtouch_tsdebug = qtouch_tsdebug & ~0x8;

	ts->client->addr = ts->org_i2c_addr;
	ts->mode = 0;
	/* Wait for the IC to recover */
	if (ts->family_id == QTM_OBP_TABLET_FAMILY_ID)
		msleep(QTM_OBP_SLEEP_WAIT_FOR_RESET_TABLET);
	else
		msleep(QTM_OBP_SLEEP_WAIT_FOR_RESET);
	for (i = 0; i < 6; i++) {
		if (i > 0)
	...
	if (err != 0) {
		...
		err = qtouch_process_info_block(ts);
		if (err != 0) {
			pr_err("%s:Fail read info block after reset%i\n",
			       __func__, err);
		}
	}
...
#ifdef CONFIG_TOUCHSCREEN_DEBUG
static int show_uptime(char *buf)
{
	struct timespec uptime;

	do_posix_clock_monotonic_gettime(&uptime);
	monotonic_to_bootbased(&uptime);
	snprintf(buf, MAX_BUFFER_SIZE, "%lu.%02lu",
			   ...
			   const struct i2c_device_id *id)
{
	...
	qtouch_force_reset(ts, 0);
	/* Add an additional delay since IC family isn't known yet */
	msleep(QTM_OBP_SLEEP_WAIT_FOR_HW_RESET_TABLET -
	...
	if (err == 0) {
		...
		if (!err) {
			...
			pr_info("%s:Bootloader version %d\n", __func__,
				boot_info[extended_boot]);

			while (touch_fw_table_ptr->family_id != 0x00) {
				if (boot_info[extended_boot] ==
				    touch_fw_table_ptr->boot_version &&
				    extended_boot ==
				    touch_fw_table_ptr->extended_boot) {
					pr_info("%s: Chip type matched\n", __func__);
					ts->family_id = touch_fw_table_ptr->family_id;
					ts->touch_fw_size = touch_fw_table_ptr->size;
					ts->touch_fw_image = touch_fw_table_ptr->image;
					break;
				}
	...
	if ((ts->touch_fw_size != 0) && (ts->touch_fw_image != NULL)) {
		/*
		 * This is a COMPLETE HACK! Enabling verbose logging during the
		 * firmware upgrade process seems to block the panel init code
		 * from running and thus saves the touch chip from being reset
		 * due to hardware bugs on Pasteur P1 tablets related to voltage
		 * drops during panel turn on.
		 */
		qtouch_tsdebug = qtouch_tsdebug | 0x8;
