...
struct cap_prox_data {
	...
	struct attiny48mu_cap_prox_platform_data *pdata;

	atomic_t enabled;
	spinlock_t irq_lock; /* Needed to protect driver state */
	int irq_state;
};
...
static u8 key1_ref_drift_thres_l;
static u8 key3_ref_drift_thres_l;
static u8 key1_ref_drift_thres_h;
static u8 key3_ref_drift_thres_h;
static u8 ref_drift_diff_thres;
...
static int cap_prox_read_data(struct cap_prox_data *cp)
{
	...
	case CP_STATUS_KEY1_IN_DETECT:
		if (key1_ref_drift < key1_ref_drift_thres_l)
	...
	case CP_STATUS_KEY3_IN_DETECT:
		if (key3_ref_drift < key3_ref_drift_thres_l)
			cap_prox_calibrate(cp);
		break;
	...
	case CP_STATUS_KEY1_KEY3_IN_DETECT:
		if ((key1_ref_drift < key1_ref_drift_thres_h) &&
			 (key3_ref_drift < key3_ref_drift_thres_h) &&
			 (ref_drift_diff < ref_drift_diff_thres))
			cap_prox_calibrate(cp);
		break;
	...
	/* Set the threshold values to defaults */
	key1_ref_drift_thres_l = cp->pdata->key1_ref_drift_thres_l;
	key3_ref_drift_thres_l = cp->pdata->key3_ref_drift_thres_l;
	key1_ref_drift_thres_h = cp->pdata->key1_ref_drift_thres_h;
	key3_ref_drift_thres_h = cp->pdata->key3_ref_drift_thres_h;
	ref_drift_diff_thres = cp->pdata->ref_drift_diff_thres;
			   ...
			   const struct i2c_device_id *id)
{
	...
	/* Disable force detect because HW variation of key1 key2 drift is too big */
	/*
	force_detect = cp->pdata->plat_cap_prox_cfg.force_detect
	...
	*/
	...
	cap_prox_write(cp, &cp->pdata->plat_cap_prox_cfg.lp_mode_active, 1);
	cap_prox_write(cp, &cp->pdata->plat_cap_prox_cfg.data_integration, 1);
	cap_prox_write(cp, &cp->pdata->plat_cap_prox_cfg.neg_drift_rate, 1);
	cap_prox_write(cp, &cp->pdata->plat_cap_prox_cfg.pos_drift_rate, 1);
	cap_prox_write(cp, &cp->pdata->plat_cap_prox_cfg.drift_hold_time, 1);
...
static int cap_prox_suspend(struct i2c_client *client, pm_message_t mesg)
{
	...
	cap_prox_irq_enable(cp, 0);
	cancel_delayed_work_sync(&cp->input_work);
	cap_prox_write(cp, &cp->pdata->plat_cap_prox_cfg.lp_mode, 1);

	/* Set the threshold values to sleep mode values */
	key1_ref_drift_thres_l = cp->pdata->key1_ref_drift_thres_l_sleep;
	key3_ref_drift_thres_l = cp->pdata->key3_ref_drift_thres_l_sleep;
	key1_ref_drift_thres_h = cp->pdata->key1_ref_drift_thres_h_sleep;
	key3_ref_drift_thres_h = cp->pdata->key3_ref_drift_thres_h_sleep;
	ref_drift_diff_thres = cp->pdata->ref_drift_diff_thres_sleep;
